...
static i32
apac_deinit (apac_ctx_t *apac_ctx)
{
  if (apac_ctx->user_session != NULL)
    apfree (apac_ctx->user_session);
  if (apac_ctx->echo_system != NULL)
    apfree (apac_ctx->echo_system);

  if (apac_ctx->core_backend != NULL)
    apfree (apac_ctx->core_backend);
  if (apac_ctx->governor != NULL)
    apfree (apac_ctx->governor);

  if (apac_ctx->root != NULL)
