...
i32 sched_start(apac_ctx_t* apac_ctx) {
	...
	echo_assert(apac_ctx, physic_cores > 0, "Pool: invalid count of cores, "
...
#define CPU_FEATURESSZ     0x70
	...
	char* cpu_vendormodel = (char*)apmalloc(sizeof(char) * CPU_VENDOR_MODELSZ);
	char* cpu_features    = (char*)apmalloc(sizeof(char) * CPU_FEATURESSZ);

	if (!cpu_realname || !cpu_vendormodel || !cpu_features) {
		echo_error(apac_ctx, "Has occured a problem when attempt to "
