...
static _Unwind_Reason_Code un_store_holder(struct _Unwind_Context* context, void* arg_data) {
	struct traceback_state* frame = (struct traceback_state*)arg_data;
	u64 program_counter = _Unwind_GetIP(context);
	if (program_counter == 0) goto fish_next;

	if (frame->curr_state == frame->end_ptr) {
		// We don't have no more memory for holder a possible new function inside our stack frame
