...
void* vec_next(vecdie_t* vec) {
	...
	if (skip_bytes > vec->vec_dsize * vec->vec_used) return NULL;
...
static u64 vec_agrow(vecdie_t* vec) {
	if (vec->vec_capa < 100) {
...
static i32 vec_copy(void* dest, void* src, vecdie_t* vec) {
	if (dest == NULL || src == NULL) return -1;

	if (vec->vec_dsize % 2 != 0 || 
		vec->vec_dsize > sizeof(long)) {
	...
	for (; ssize ; ssize--) *dptr++ = *sptr++;

	return 0;
}

static inline i32 vec_grow(vecdie_t* vec) {
	if (vec->vec_capa > vec->vec_used) return 0;

	const i32 resize = vec_resize(vec_agrow(vec), vec);
	return resize;
}

void* vec_at(u64 index, vecdie_t* vec) {
	if (index > vec->vec_capa) return NULL;
	u64 addr_index = vec->vec_dsize * index;
	return vec->vec_dynamic + addr_index;
}

i32 vec_push(void* item, vecdie_t* vec) {
	if (vec_grow(vec) != 0) return -1;

	const i32 vec_copied = vec_copy(vec_at(vec->vec_used++, vec), item, vec);

	if (vec_copied != 0) return vec_copied;

	return 0;
}

void* vec_emplace(vecdie_t* vec) {
	if (vec_grow(vec) != 0) return NULL;
