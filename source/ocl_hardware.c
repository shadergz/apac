...
#define AP_LOAD_FUNC(ptr, sym_name, sym_type, lib_name, onerr)\
	...
	ptr->sym_name = (sym_type)sys_loadsym(ptr->ocl_driver, #sym_name)
...
i32 ocl_init(apac_ctx_t* apac_ctx) {
	...
	ocl_int->ocl_driver = sys_loadobject(fio_getpath(be_context->ocl_shared)); 
