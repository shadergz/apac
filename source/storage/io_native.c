...
native_flags_t native_solve_flags(const char* flags) {
	...
	if (read && write) ret  |= STORAGE_FLAG_RW;
	if (!read && write) ret |= STORAGE_FLAG_WRITE;

	if (file) ret |= STORAGE_FLAG_FILE;
	if (dir)  ret |= STORAGE_FLAG_DIRECTORY;

	if (create) ret |= STORAGE_FLAG_CREAT;

	return ret;
}

static i32 native_solver(char* solver) {
	char* hiphen = strchr(solver, '-');
	if (hiphen == NULL) return -1;

	u64 location = 3 - (u64)(hiphen - solver);
	if (!location) return -1;
	while (location-- != 0) *hiphen++ = '0';
