...
#define FIO_IS_REFERENCE(perm)\
	(strncasecmp(perm, "ref", 3) == 0)
...
const u64 fio_read(storage_fio_t* file, void* data, u64 datas) {
	...
	if (datas <= file->cache_valid) {
		echo_assert(NULL, (file->cache_cursor - 
			file->rw_cache) >= file->cache_valid, "Something is going "
	...
	do {
		...
		if (file->cache_cursor - file->rw_cache > sizeof(file->rw_cache)) {
			const u64 inptr = (file->cache_cursor - 
				file->rw_cache) % sizeof(file->rw_cache);
...
i32 fio_seekbuffer(storage_fio_t* file, u64 offset, fio_seek_e seek_type) {
	switch (seek_type) {
	case FIO_SEEK_SET:
