...
i32 dirio_rewind(storage_dirio_t* dir) {
	if (dir->dir_fd < 3) return -1;
	return dir->buf_pos = dir->buf_end = dir->position = lseek(dir->dir_fd, 0, SEEK_SET);
}

u64 dirio_read(void* store_local, u64 buf_size, storage_dirio_t* dir) {
	if (dir->dir_fd < 3 || store_local == NULL || buf_size == 0) return 0;
	
	if (__builtin_expect((dir->buf_pos >= dir->buf_end), 0)) {
		const u64 dir_len = dirio_getentries(dir->dir_fd, 
				       (native_dirent_t*)dir->dir_info, sizeof dir->dir_info);
		dir->buf_end = dir_len;
		dir->buf_pos = 0;
	...
	const native_dirent_t* entity_info = (const native_dirent_t*)
		(dir->dir_info + dir->buf_pos);
