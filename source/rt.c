...
static i32
rt_dump (u64 idx, bool valid_trace, void *user_data, const char *format, ...)
{
  va_list va;
  va_start (va, format);
...
static void
thread_segv_handler (int signum, siginfo_t *info, void *context)
{
...
#define X86_PC_LOCATION 1 << 4
  ...
  const register_t *readable_regs
      = (const register_t *)prog_context->uc_mcontext.gregs;
  ...
  for (u8 reg_index = 0;
       reg_index < sizeof (registers_name) / sizeof (const char *);
       reg_index++)
    {
      echo_error (NULL, "\033[0;38m%s = \033[0;32m%016llx\033[0m\n",
                  registers_name[reg_index], readable_regs[reg_index]);
    }
...
void
run_raisef (i32 rsig, const char *fmt, ...)
{
...
i32
run_getedir (char **save_path, u64 req_sz)
{
  ...
  if (save_path == NULL)
    return -1;
  if (read_path == NULL || *save_path != NULL)
    return -1;
