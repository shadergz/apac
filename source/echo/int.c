...
static bool echo_canlogger(i32 level, user_options_t* user) {
	...
	if (nice == 0) return true;

	return false;
}

static i32 echo_default(void* apac_addr, i32 level, const char* message, 
...
static i32 echo_release(apac_ctx_t* apac_ctx, const char* message, 
		echo_level_e level, const schedthread_t* thread_log) {
	echo_ctx_t* event = NULL;
	if (apac_ctx != NULL)
		event = apac_ctx->echo_system;

	i32 wrote = 0;
	if (event == NULL) goto use_default;

	if (event->event_announce != NULL) {
		wrote = event->event_announce((void*)apac_ctx, (i32)level, 
				message, "Invalid");
		goto done;
	}
...
i32 echo_do(apac_ctx_t* apac_ctx, echo_level_e msg_level, i32 code_line, 
	...
	if (apac_ctx != NULL) {
		...
		if (echo_user == NULL) goto echo_finish;

		if (echo_canlogger((i32)msg_level, echo_user) != 0) return -1;
	}
